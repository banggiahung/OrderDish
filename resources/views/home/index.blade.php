            ...
            methods: {
                filteredDishesMeal() {
                    ...
                    if (this.dataSubmit.meal) {
                        ...
                        // If no meal or restaurant is selected, return all dishes
                        return this.dish;
                    }
                },
                filteredDishesMealRestaurant(){
                    ...
                    if (this.dataSubmit.restaurant) {
                        ...
                        // Cập nhật danh sách các nhà hàng
                        return filteredByRestaurant;
                ...
                filteredDishesAdd() {
                    ...
                    // Kiểm tra xem tên của món ăn mới đã chọn có trùng với các món ăn đã chọn trước đó hay không
                    if (this.dataSubmit.dish.some(item => item.dish.name === newDishName && item !== this.dataSubmit.dish[this.dataSubmit.dish.length - 1])) {
                        ...
                        // Nếu trùng tên, loại bỏ món ăn mới đã chọn
                        this.dataSubmit.dish.pop();
                    }

                },
                initData() {
                    fetch('/dishes.json') // Assuming the JSON file is served from the root directory
                        .then(response => response.json())
                        .then(data => {
                            this.dish = data.dishes;
                            this.dishMain = data.dishes;
                            // Extract unique meals and restaurants from dishes
